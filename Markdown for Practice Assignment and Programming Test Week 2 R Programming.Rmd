...
Adapting for question 2

question_2 <- function(directory, id) {
...
print(needed_files)
     ...
     for (i in seq_along(needed_files)) {
         dat <- rbind(dat, read.csv(needed_files[i]))
     }
...
Trying the more correct approach to answer 1

checking to see if the 1st part works

just_list<- function(directory){
a_directory <- list.files(directory, full.names=TRUE)
a_directory
...
empty_df_and_subset <- function(directory,id){
a_directory <- list.files(directory, full.names=TRUE)

empty_df <- data.frame()

directory <- dir(a_directory)
directory
...
This is so goddamn close to the answer for 2
 ...
 total_obs <- function(directory,id){
     ...
     #Now we need to create a data frame that groups by ID
     #And only includes complete cases
...
blehh <- for (i in seq_along(full_data_set)) {
         dat <- rbind(dot, read.csv(full_data_set[i]))
         print(nrow(dat))}
...
This is so goddamn close to the answer for 2
 ...
 total_obs <- function(directory,id){
     dot <- data.frame()
     for (i in directory[1:2]) {
         dat <- rbind(dot, read.csv(directory[i]))
     }
     print(nrow(dat))
     hmm <- dat[which(dat[, "ID"] %in% id),]
     print(nrow(hmm))
